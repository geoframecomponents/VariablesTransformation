...
public class InverseZScore {

	@Description("Data set of observed variable to transformed with z-score")
	@In
	public HashMap<DateTime, double[]> inData;
	
	@Description("Data set of z-score variable to transformed with the inverse z-score")
	@In
	public HashMap<DateTime, double[]> inDataZScore;
	...
	public void run() {
		...
		for(DateTime dateTime : inDataZScore.keySet()) {
			
			tmp1 = inDataZScore.get(dateTime);
			tmp2 = new double[tmp1.length];

			for(int i=0; i<tmp1.length; i++) {
				
				if(!Double.isNaN(tmp1[i])) {
					tmp2[i] = tmp1[i]*standardDeviation + mean;
